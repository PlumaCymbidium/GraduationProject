...
def get_img_path(target_dir, dataset):
    ...
    if dataset == 'DRIVE':
        ...
        img_files, vessel_files, mask_files = proed_files(target_dir)
...
def proed_files(data_path):
    img_dir = os.path.join(data_path, "images")
    vessel_dir = os.path.join(data_path, "1st_manual")
    mask_dir = os.path.join(data_path, "mask")

    img_files = all_files_under(img_dir, extension=".bmp")
    vessel_files = all_files_under(vessel_dir, extension=".bmp")
    mask_files = all_files_under(mask_dir, extension=".bmp")
